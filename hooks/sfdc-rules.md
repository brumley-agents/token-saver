...
Injected at session start. They cover Salesforce / ticketing specifics that a general-purpose output style can't know. General terseness (response shape, tool-use hygiene, when to break terseness) lives in `output-styles/terse.md` — activate it with `/output-style terse` when you want it; these directives don't depend on that style and apply regardless.
